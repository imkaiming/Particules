...
Grain::Grain()
    : durationSamples{0}, startPositionSamples{0}, speed{1.f}, sustainWidthSamples{0}, envelopeType{0}, envelopeSizeSamples{0},
      selectionWindow{0}, fadeIn{0}, fadeOut{0}, elapsedSamples{0}, delaySamples{0}, readPosition{0.f}, inputNumChannels{0},
...
void Grain::config(const ParameterSnapshot& snapshot, int delay, float normalizedPosMod)
{
    ...
    fadeIn = static_cast<int>(0.5f * (durationSamples - sustainWidthSamples));
    fadeOut = fadeIn + sustainWidthSamples;
    fadeIn = std::max<int>(fadeIn, 1);
    fadeOut = std::max<int>(fadeOut, 1);
...
float Grain::getCurrentSample(const AudioBuffer* inputbuffer, const int outChannel, const int outNumChannels) noexcept
{
    if(delaySamples-- > 0)
        return 0.f;
...
void Grain::tukeyEnvelope(std::vector<float>& table)
{
    const float invFadeIn = fadeIn > 0 ? 1.f / fadeIn : 0.f;
    const float invFadeOut = (durationSamples - fadeOut) > 0 ? 1.f / (durationSamples - fadeOut) : 0.f;
    ...
    for(int i = 0; i < durationSamples; ++i)
    {
        if(i < fadeIn) // fade in
        {
            float p = i * invFadeIn; // 0 → 1
            table[i] = 0.5f * (1.f - std::cos(pi * p));
        }
        else if(i >= fadeOut) // fade out
        {
            float p = (i - fadeOut) * invFadeOut; // 0 → 1
            table[i] = 0.5f * (1.f + std::cos(pi * p));
        }
        else // plateau
        {
