
#pragma once
#include "../framework/pch.h"

namespace particules
{
    namespace colours
    {
        
        //inline const juce::Colour grey{0xff0a0a0a}; // https://www.color-hex.com/color/0a0a0a
        inline const juce::Colour deepBlack{0xff1f1f1f}; // https://www.color-hex.com/color/1f1f1f

        // black tone
        inline const juce::Colour smokyBlack{0xff100c08};
        inline const juce::Colour grey{0xffa0a0a0};
        inline const juce::Colour midGrey{0xff666666};
        inline const juce::Colour darkGrey{0xff353535}; 
        inline const juce::Colour black{0xff141414};

        // accent colo
        inline const juce::Colour green{0xff52b788};
        inline const juce::Colour lavender{0xffccccff};
        inline const juce::Colour blue{0xff6dd4ff};
        inline const juce::Colour cream{0xfff6f9e4};
        inline const juce::Colour red{0xffe5383b};
        inline const juce::Colour pink{0xffff8fa3};
        inline const juce::Colour jungleGreen{0xff2AAA8A};
        inline const juce::Colour brightBlue{0xff0096FF};
    }
}